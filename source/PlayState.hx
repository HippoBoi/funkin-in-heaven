...
class PlayState extends MusicBeatState
{
	...
	var weekclear:FlxSprite = new FlxSprite(400, -50);
				...
				case 'heaven1':
					{
						...
						var cloud:FlxSprite = new FlxSprite(-195, -95).loadGraphic(Paths.image('funkinHeaven/skyBG1'));
						cloud.scrollFactor.set(0.1, 0.1);
						cloud.setGraphicSize(Std.int(cloud.width * 3));
						cloud.setGraphicSize(Std.int(cloud.height * 4.8));
						cloud.updateHitbox();
						add(cloud);
						
						var cloud:FlxSprite = new FlxSprite(-195, -80).loadGraphic(Paths.image('funkinHeaven/skyBG12'));
						cloud.scrollFactor.set(0.2, 0.2);
						cloud.setGraphicSize(Std.int(cloud.width * 3.8));
						cloud.setGraphicSize(Std.int(cloud.height * 4.8));
						cloud.updateHitbox();
						add(cloud);
						
						var cloud:FlxSprite = new FlxSprite(-255, -30).loadGraphic(Paths.image('funkinHeaven/skyBG13'));
						cloud.scrollFactor.set(0.9, 0.9);
						cloud.setGraphicSize(Std.int(cloud.width * 4.1));
						cloud.setGraphicSize(Std.int(cloud.height * 5.1));
						cloud.updateHitbox();
						add(cloud);
					}
				...
				case 'heaven2':
					{
						...
						var cloud:FlxSprite = new FlxSprite(-55, 0).loadGraphic(Paths.image('funkinHeaven/cloudBG1'));
						cloud.scrollFactor.set(0.4, 0.4);
						cloud.setGraphicSize(Std.int(cloud.width * 3.8));
						cloud.setGraphicSize(Std.int(cloud.height * 3.8));
						cloud.updateHitbox();
						add(cloud);
	...
	function endSong():Void
	{
		...
		if (isStoryMode)
			...
			if (PlayState.SONG.song.toLowerCase() == 'lihtronx') //THIS WORKS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
		{
			//final scene then go back to storymenu
	...
	override function beatHit()
	{
		...
		if (curBeat == 22 && curSong == 'test')
		{
			defaultCamZoom = 1.2;
		
			FlxG.sound.play(Paths.sound('victory'), 1);
		}
		
		if (curBeat == 26 && curSong == 'test')
		{
			defaultCamZoom = 1.1;
			weekclear.frames = Paths.getSparrowAtlas('funkinHeaven/weekclear');
			weekclear.animation.addByPrefix('clear','weekclear',24, false);
			
			weekclear.screenCenter();
			
			add(weekclear);
			weekclear.animation.play('clear');
			
			
			boyfriend.playAnim('hey', true);
		}
		
		if (curBeat == 32 && curSong == 'test')
		{
			defaultCamZoom = 0.5;
			remove(boyfriend);
			remove(dad);
